...
from real import get_transform_signfor, get_transform
...
def read_images_signformer(folder_path, frames=16):
    assert len(os.listdir(folder_path)) >= frames, "Too few images in your data folder: " + str(
        folder_path)
...
def predict(file_name, selected_model, selected_weight, agree, video_style, centercrop):
    ...
    if (selected_model=="r2+1d_100") | (selected_model=="r2+1d_500"):
        ...
        model.load_state_dict(torch.load(model_path))
        model.eval()
    elif selected_model=="SignFormer_100":
        model_path = "weight/" + str(selected_model) + "/" + str(selected_weight)
        model = SignFormer(num_classes=num_classes)
    ...
    if selected_model in ("SignFormer_100", "SignFormer_500"):
        images = read_images_signformer(data_path)
        images = torch.reshape(images, (1, 3, 16, 224, 224))
    else:
