...
def image_to_video(sample_dir, video_name):
    """
    sample_dir: 图片帧目录（如 'data/images'），要求有 %06d.jpg 命名
    video_name: 输出视频全路径，建议以 .mp4 结尾（如 'data/videos/output.mp4'）
    """
    # 如果你只想保留最近20帧，建议这段移到推理后而不是前面。否则有数据丢失风险。
    # ls = sorted(os.listdir(sample_dir))
    # for i in ls[:-20]:
    #     os.remove(os.path.join(sample_dir, i))

    # 确保 ffmpeg.exe 路径正确（推荐绝对路径）
    ffmpeg_path = r"D:\softwares\ffmpeg\ffmpeg-7.1.1-full_build\bin\ffmpeg.exe"  # 替换为你的实际路径
    img_pattern = os.path.join(sample_dir, "%06d.jpg")
    output_path = video_name if video_name.endswith('.mp4') else video_name + '.mp4'
